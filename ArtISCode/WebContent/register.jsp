...
$(document).ready(function(){
	...
	$("#loginBox").formValidator({onshow:"please input your name\"maodong\"is correct",onfocus:"6-10 letters",oncorrect:"you can register"}).inputValidator({min:6,max:10,onerror:"illegal user Name"}).regexValidator({regexp:"username",datatype:"enum",onerror:"not correct user name format"})
	.ajaxValidator({
	    type : "post",
		url : "CheckLogin",
		datatype : "html",
		...
		buttons: $("#button"),
		error: function(){alert("the server is busy");},
		onerror : "the user name is not availabe, try another one",
		onwait : "check if the name is correct, please wait..."
	});
	$("#pass1").formValidator({onshow:"input password",onfocus:"password can't be empty",oncorrect:"ok"}).inputValidator({min:1,empty:{leftempty:false,rightempty:false,emptyerror:"no blanks on the both sides of the password"},onerror:"password can't be empty"});
	$("#pass2").formValidator({onshow:"input again",onfocus:"two passwords must be the same",oncorrect:"ok"}).inputValidator({min:1,empty:{leftempty:false,rightempty:false,emptyerror:"no blanks on the both sides of the password"},onerror:"password can't be empty"}).compareValidator({desid:"pass1",operateor:"=",onerror:"two passwords must be the same"});
});
--></script>
</head>
<body>
