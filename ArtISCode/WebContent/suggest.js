...
function  handleResult() {
	if (searchReq.readyState == 4) {
		...
		while(i < str.length - 1) {
			...
			+'<h3>'+ str[i] + ' ' +str[i+1] +'</h3>';
			suggest += '<div id="searchdesc">';
			if (str[10]==" "){
				suggest += '<img width="180" height="240" border=no src="Img/default_big.gif"/>';
			}
			else {
				suggest += '<img width="180" height="240" border=no src="'+ str[i+3] +'"/>';
			}
			suggest += '<p>' + str[i+4] + '</p></div>';
